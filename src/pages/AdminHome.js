...
export default function AdminHome({ onLogout }) {
  ...
  // EXCLUSÃO SIMPLIFICADA (sem tentar excluir rotas primeiro)
  ...
  const handleAtribuirRota = async (cliente) => {
    if (!entregadorSelecionado) { toast.warning('Selecione entregador'); return; }
  ...
  return (
    <div style={{ ...styles.container, backgroundColor: cores.background }}>
      <div style={styles.header}>
        ...
        <ProfileMenu usuario={{ nome: 'Administrador', tipo: 'admin' }} onLogout={onLogout} toggleTheme={toggleTheme} onNavigate={handleNavigate} />
      </div>
          ...
          {clientesFiltrados.map(c => {
            ...
            return (
              <div key={c.id} style={{ ...styles.card, backgroundColor: cores.card, borderColor: emRota ? cores.success : cores.cardBorder }}>
                <div>
                  <p><strong>{c.nome}</strong> {emRota && <span style={{ color: cores.success }}>🚚 Em rota</span>}</p>
                  <p>📞 {c.telefone}</p>
                  <p>📍 {c.endereco}</p>
                  <p>🔑 {c.codigoEntrega} | Pedidos: {c.qtdPedidos}</p>
                </div>
                <div style={{ display: 'flex', gap: 6, flexDirection: 'column' }}>
                  ...
                  <button style={styles.botaoDeletar} onClick={() => handleExcluirCliente(c.id, c.nome)}>🗑️</button>
                </div>
              </div>
            );
          })}
      ...
      {/* Abas restantes (rotas, historico, entregadores) - omitidas por brevidade, mas pode manter igual */}
    </div>
  );
}

const styles = {
  container: { padding: 16, minHeight: '100vh' },
  header: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16, flexWrap: 'wrap' },
  titulo: { fontSize: 24 },
  ...
  botaoEditar: { backgroundColor: '#2a2a4a', border: 'none', padding: 8, borderRadius: 8, cursor: 'pointer' },
